...
impl ComponentRegistry {
    ...
    pub fn draw_for_entity(&mut self, gui: &mut Gui, rect: Rect, state: &mut EditorState) {
        ...
        for editor in &mut self.editors {
            let section_rect =
                Rect::from_min_size(Pos2::new(rect.left(), y), Vec2::new(rect.width(), 200.0));
...
fn draw_inspector_components(gui: &mut Gui, rect: Rect, _state: &mut EditorState) {
    ...
    let mut render = RenderEditor {
        material: "Default".into(),
        mesh: "Cube".into(),
        cast_shadow: true,
    };
    let mut physics = PhysicsEditor {
        body_type: "Static".into(),
        collision_shape: "Box".into(),
    };
