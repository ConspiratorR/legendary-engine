        ...
        if let Some(time) = app.resources.get::<Time>() {
            if time.frame_count() % 60 == 0 {
                if let Some(config) = app.resources.get::<Config>() {
                    ...
                    println!(
                        "Game Title: {}",
                        config.get("game.title").map_or("Unknown", |v| v)
                    );
                    println!(
                        "Version: {}",
                        config.get("game.version").map_or("0.0.0", |v| v)
                    );
                    println!(
                        "Player Health: {}",
                        config.get("player.health").map_or("100", |v| v)
                    );
                    println!(
                        "Player Speed: {}",
                        config.get("player.speed").map_or("1.0", |v| v)
                    );
                }
