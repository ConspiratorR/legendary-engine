...
| P3.3 | 场景引用资产 | `serialization.rs` | SceneData 组件属性支持 guid 引用 ScriptableObject |
| P3.4 | 与 engine-asset 对齐 | 调研 `engine-asset` 的 Handle/GUID | 文档写清两套资产系统关系；能共存则桥，不能则标为 Phase 6 |
| P3.5 | 工具：`so_asset pack/unpack` | 可选 bin | CLI 批量扫描生成 meta |
