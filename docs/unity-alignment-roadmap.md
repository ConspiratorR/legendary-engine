...
| ID | 任务 | 说明 |
|----|------|------|
| P2.4 | 将 `gameobject_data`/`transforms`/`monobehaviours` 迁入 ECS 资源或组件 | 切片：flag；World 自持 identity；Name/Tag/Active/Transform/MonoBehaviourTypes 写内部 ECS；数组存储迁移仍后续 |
